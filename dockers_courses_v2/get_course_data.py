...
def get_categories():
    ...
    categories = pd.merge(categories,categ_lvl_1[['id','name']],left_on=['lvl_3'],right_on=['id'],how='left')
    categories =  categories.rename(columns={'name':'category_lvl3'})
    categories.drop(columns=['id'],inplace=True)
    return categories
...
def complement_courses(courses, categories):
    courses = pd.merge(courses,categories[['categoryid','category_name','lvl_1','lvl_2', "lvl_3",'category_lvl1','category_lvl2', "category_lvl3","depth", "parent", "path", "lenght", "category_visible"]],left_on="category", right_on= 'categoryid',how='left')
...
def main():
    ...
    compose_table(courses_complete, "courses_planestic_analytics_v2")
    logger.info("Insert Data")
    insert_courses_data(courses_complete, "courses_planestic_analytics_v2")
